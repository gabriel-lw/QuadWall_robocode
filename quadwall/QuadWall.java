...
//	double[][][] trackList =  new double[3][8][2];
	...
	//separar os nomes por quadrante, possibilitando detectar mudança de quadrante
	...
	public void scanOpponentQuadrant(ScannedRobotEvent e) {
		...
		if (names.contains(e.getName())) {
			...
			//e se foi contado agora em outro quadrante?
			//verificar novamente x e y
		}
		...
		// Se robô oponente já não foi escaneado, verificar qual quadrante ele está
			...
			if (opponentX < quadrantBorderX &&  opponentY > quadrantBorderY) {
				...
				//verificar aqui se o quadrante ja possui esse nome
				//se for adicionado por mudança de quadrante, precisa retirar ele da contagem do quadrante anterior
				numFirstQuadrant += +1;
	...
	public void run() {
			...
			if (distanceWestWall < distanceEastWall && distanceWestWall < distanceNorthWall && distanceWestWall < distanceSouthWall) {
				...
				/*
					trackQuad[0][0] = trackList[borderLV][1][0];
					trackQuad[0][1] = trackList[borderLV][1][1];
					trackQuad[1][0] = trackList[borderLV][0][0];
					trackQuad[1][1] = trackList[borderLV][0][1];
					trackQuad[2][0] = trackList[borderLV][7][0];
					trackQuad[2][1] = trackList[borderLV][7][1];*/
...
/*
					trackQuad[0][0] = trackList[borderLV][1][0];
					trackQuad[0][1] = trackList[borderLV][1][1];
					trackQuad[1][0] = trackList[borderLV][2][0];
					trackQuad[1][1] = trackList[borderLV][2][1];
					trackQuad[2][0] = trackList[borderLV][3][0];
					trackQuad[2][1] = trackList[borderLV][3][1];*/
			...
			// Se estiver mais próximo da borda leste, direcionar para ela
				...
				/*
					trackQuad[0][0] = trackList[borderLV][5][0];
					trackQuad[0][1] = trackList[borderLV][5][1];
					trackQuad[1][0] = trackList[borderLV][6][0];
					trackQuad[1][1] = trackList[borderLV][6][1];
					trackQuad[2][0] = trackList[borderLV][7][0];
					trackQuad[2][1] = trackList[borderLV][7][1];*/
				...
				/*
					trackQuad[0][0] = trackList[borderLV][5][0];
					trackQuad[0][1] = trackList[borderLV][5][1];
					trackQuad[1][0] = trackList[borderLV][4][0];
					trackQuad[1][1] = trackList[borderLV][4][1];
					trackQuad[2][0] = trackList[borderLV][3][0];
					trackQuad[2][1] = trackList[borderLV][3][1];*/
			...
			// Se estiver mais próximo da borda norte, direcionar para ela
				...
				/*
					trackQuad[0][0] = trackList[borderLV][3][0];
					trackQuad[0][1] = trackList[borderLV][3][1];
					trackQuad[1][0] = trackList[borderLV][2][0];
					trackQuad[1][1] = trackList[borderLV][2][1];
					trackQuad[2][0] = trackList[borderLV][1][0];
					trackQuad[2][1] = trackList[borderLV][1][1];*/
					...
					trackQuad[1][0] = trackList[borderLV][4][0];
					trackQuad[1][1] = trackList[borderLV][4][1];
					trackQuad[2][0] = trackList[borderLV][5][0];
					trackQuad[2][1] = trackList[borderLV][5][1];
			...
			// Se estiver mais próximo da borda sul, direcionar para ela
				...
				if(distanceWestWall <= distanceEastWall )
				{
					...
					idCurrentQuad = 0;
					//definir dps o numero da borda, nao 0
					/*trackQuad[0][0] = trackList[borderLV][7][0];
					trackQuad[0][1] = trackList[borderLV][7][1];
					trackQuad[1][0] = trackList[borderLV][0][0];
					trackQuad[1][1] = trackList[borderLV][0][1];
					trackQuad[2][0] = trackList[borderLV][1][0];
					trackQuad[2][1] = trackList[borderLV][1][1];*/
	...
	void changeQuadrant(int quad){
			...
			//aim quadrant ser o final, pegar o id 1 do meio
		//	nerest verificar a mudança entre quadrantes intermediarios
			
			//se cair que o nextTrack do quadrante nao for mais proximo, verificar o proximo quadrante(intermediario) e mudar para esse quadrante e assim por diante
	...
	//	if(robotX -tracklist[borderLV][idCurrentQuadrant][nearestId][0] <
		//tracklist[borderLV][aimmQuadrant][nearestId][0]
	}
	...
	void setQuadTrack(){
		...
		//metodo separado desse para o set inicial
		//
